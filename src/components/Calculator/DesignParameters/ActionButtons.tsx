...
const ActionButtons: React.FC<ActionButtonsProps> = ({
  ...
  return <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
      <div className="flex-1">
        <Button variant="outline" onClick={() => setShowOtherParams(!showOtherParams)} className="w-full">
          {showOtherParams ? 'Hide Other Parameters' : 'Show Other Filter Design Parameters'}
        </Button>
      </div>
      <div className="flex-1">
        <Button variant="outline" className="w-full" onClick={() => setShowDimensions(!showDimensions)}>
